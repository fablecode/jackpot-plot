using DbUp;
using DbUp.Engine;
using JackpotPlot.Lottery.API.DatabaseMigration.Exceptions;
using Microsoft.Extensions.DependencyInjection;
using Serilog;
using System.Data.Common;
using System.Reflection;

namespace JackpotPlot.Lottery.API.DatabaseMigration
{
            ...
            try
            {
                var upgradeEngine =
                    DeployChanges.To
                        ...
                        .WithScriptsEmbeddedInAssembly(Assembly.GetExecutingAssembly(), script => script.StartsWith("JackpotPlot.Lottery.API.DatabaseMigration.Scripts.PreDeployment."), new SqlScriptOptions { RunGroupOrder = 1 })
                        // Migration scripts
                        .WithScriptsEmbeddedInAssembly(Assembly.GetExecutingAssembly(), script => script.StartsWith("JackpotPlot.Lottery.API.DatabaseMigration.Scripts.Migrations."), new SqlScriptOptions { RunGroupOrder = 2 })
                        // PostDeployment scripts
                        .WithScriptsEmbeddedInAssembly(Assembly.GetExecutingAssembly(), script => script.StartsWith("JackpotPlot.Lottery.API.DatabaseMigration.Scripts.PostDeployment."), new SqlScriptOptions { RunGroupOrder = 3 })
