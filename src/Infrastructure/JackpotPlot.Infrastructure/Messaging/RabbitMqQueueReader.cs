...
public class RabbitMqQueueReader<T> : IQueueReader<T>
{
    ...
    public async ValueTask Subscribe(string queueName, HashSet<string> routingKeys, Channel<(T message, TaskCompletionSource<Result<T>> tcs)> channel, CancellationToken cancellationToken)
    {
        _logger.LogInformation("Starting subscription to queue: {QueueName}, with routing keys {RoutingKeys}", queueName, routingKeys);
        ...
        await rabbitMqChannel.BasicConsumeAsync(queue: queueName, false, consumer: consumer, cancellationToken: cancellationToken);
        _logger.LogInformation("Started consuming messages from queue: {QueueName}, with routing keys {RoutingKeys}", queueName, routingKeys);
